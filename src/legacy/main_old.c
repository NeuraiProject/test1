...
void get_address_from_output_script(unsigned char* script, int script_size, char* out, int out_size) {
    ...
    //Loose check
    if (btchip_output_script_is_regular_neurai_asset(script)) {
        addressOffset = 4;
...
uint8_t prepare_single_output() {
    ...
    if ((type = btchip_output_script_try_get_neurai_asset_tag_type(btchip_context_D.currentOutput + offset,  sizeof(btchip_context_D.currentOutput) - offset)) >= 1) {
      if (type <= 3) {
        ...
        if (type == 1) {
          ...
          offset += 1;
          // Validity of string checked in btchip_output_script_try_get_neurai_asset_tag_type
    ...
    asset_ptr = btchip_output_script_get_neurai_asset_ptr(
    ...
    if (asset_ptr > 0) {
      ...
      // Asset is verified to be ascii (signed char) safe in btchip_output_script_get_neurai_asset_ptr
