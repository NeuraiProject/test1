...
static bool check_output_displayable() {
    ...
    if (nullAmount) {
        ...
        tag_type = btchip_output_script_try_get_neurai_asset_tag_type(
            btchip_context_D.currentOutput + 8,
            sizeof(btchip_context_D.currentOutput) - 8
        );
    }
    ...
    if (asset_ptr > 0) {
        isP2sh = btchip_output_script_is_p2sh_neurai_asset(btchip_context_D.currentOutput + 8);
        asset_type = (btchip_context_D.currentOutput + 8)[asset_ptr];
        invalid = !btchip_output_script_is_regular_neurai_asset(btchip_context_D.currentOutput + 8) && 
